        ...
        try(IloCplex cplex = new IloCplex()){
            ...
            for (int i = 1; i < smartPoints; i++) {
                for (int j = 0; j < smartPoints; j++) {
                    if (i != j) {
                        ...
                        for (int q = 0; q < parcels; q++) {
                            if (locationsPerParcel.get(q).contains(j))
                                unloadedParcels.addTerm(1.0, delivery[q][j]);
                        }

                        cplex.addGe(time[i], cplex.sum(
                                cplex.sum(cplex.sum(cplex.prod(loadingCost, unloadedParcels), time[j]), travelMatrix[j][i]),
            ...
            if (cplex.solve()) {
                ...
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilename))) {
                    ...
                    for (int i = 0; i < smartPoints; i++){
                        for (int j = 0; j < smartPoints; j++)
                            transportationCost += cplex.getValue(route[i][j]) * travelMatrix[i][j];
                    }
