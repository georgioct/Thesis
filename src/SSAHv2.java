...
/**
 * Class which solves the Stochastic Savings Heuristic Vehicle Routing Problem. We have:
 ...
 * we have at most smartPoints number of routes ( 0->1-> 0 , 0->2->0 e.t.c.), we implement the stochastic savings heuristic for
 * route merging.
 * *
...
public class SSAHv2 {
     ...
     */
    public SSAHv2(String SPFilename, String parcelFilename, String outputFilename, String properties) {
     ...
     * It maximizes the number of delivered parcels and applies the stochastic savings heuristic method to merge routes.
     */
    private void solve() {
    ...
    private void savingsApproach(HashMap<Integer, ArrayList<Long>> parcelsPerLocation, HashMap<Integer,int[]> sizesPerLocation) {
        ...
        // used in order to compare the transportationCost from each iteration //
        double bestTransportationCost = Double.MAX_VALUE;
        // parameter for the softMax probabilities //
        double beta = 0.7;
        ...
        for (int iteration = 0; iteration < 100; iteration++) {
            ...
            for (Integer key : routes.keySet()) {
                copyRoutes.put(key, new ArrayList<>(routes.get(key)));
            }

            // create a deep copy for the savingsList //
            ArrayList<Saving> copySavings = new ArrayList<>();
            for (Saving saving : savingsList)
                copySavings.add(new Saving(saving.i,saving.j, saving.saving));

            // while we still have saving pairs to check //
            while (!copySavings.isEmpty()) {

                // get index for selection of saving pair through random selection from softMax //
                int softMaxSelection = softMaxSelection(copySavings,beta);

                Saving saving = copySavings.get(softMaxSelection);

                int start = saving.i;
                int end = saving.j;

                List<Integer> routeStart = findRoute(copyRoutes, start);
                List<Integer> routeEnd = findRoute(copyRoutes, end);
                ...
                if (routeStart != routeEnd && canMerge(start, end, Objects.requireNonNull(routeStart), Objects.requireNonNull(routeEnd), sizesPerLocation, parcelsPerLocation))
                    mergeRoutes(copyRoutes, routeStart, routeEnd, start, end);

                // remove the saving pair from the list //
                copySavings.remove(softMaxSelection);
            }

            // compute the transportation Cost of the produced solution//
            double transportationCost = computeTransportationCost(copyRoutes);

            // check if this solution beats the current best //
            if (transportationCost < bestTransportationCost) {
                // update current best solution //
                bestTransportationCost = transportationCost;
                // no need to create a deep copy here //
                bestRoutes = new HashMap<>(copyRoutes);
            }
        }

        writeArrivingTimes(bestRoutes, parcelsPerLocation);

        writeRoutes(bestRoutes);

        writeTransportationCost(bestRoutes);
    }

    /**
     * Method which helps us compute the transportation Cost of a List of routes.
     * @param routes ,refers to the routes of the suggested solution.
     * @return double ,the transportation cost.
     */
    private double computeTransportationCost(HashMap<Integer, List<Integer>> routes) {
    ...
    /**
     * Method used for randomly selecting a savingsList pair.
     * @param savingsList ,refers to a List containing all the savings pairs.
     * @param beta ,parameter for the softMax function - applies randomness to the selection.
     * @return index of chosen pair of saving.
     */
    private int softMaxSelection(ArrayList<Saving> savingsList, double beta) {

        // get the softmax probabilities of choosing each pair from savings list //
        double[] probabilities = softmax(savingsList,beta);
        // choose a random double from (0,1) //
        double random = new Random().nextDouble();

        // we will select a specific pair based on the random value //
        double sum = 0.0;
        ...
        // it will never reach here, just for compiler //
        return -1;
    }

    /**
     * Method which returns the probabilities of choosing each on af the savings pairs.
     * It applies the softMax function for transforming double values into probabilities distribution.
     * @param savingsList ,refers to a List containing all the savings pairs.
     * @param beta ,parameter for the softMax function - applies randomness to the selection.
     * @return array of probabilities of choosing each pair of savingsList.
     */
    private double[] softmax(ArrayList<Saving> savingsList, double beta) {

        double[] probabilities = new double[savingsList.size()];
        double sum = 0.0;

        // compute the softmax probability for each saving pair //
        for (int i = 0; i < savingsList.size(); i++){
            probabilities[i] = Math.pow(savingsList.get(i).saving,beta);
            sum += probabilities[i];
        }
    ...
    private void writeTransportationCost(HashMap<Integer, List<Integer>> routes) {
        ...
        double transportationCost = computeTransportationCost(routes);
    ...
    public static void main(String[] args) {
        ...
        SSAHv2 vrp = new SSAHv2(SPFilename,parcelFilename,outputFilename,mipProperties);
        vrp.execute();
    }
}